...
> SHM 平台后端 v0.9.3 · 更新于 2026-08-13
>
...
**禁止修改 `base.py` 中的 `ProtocolAdapter` 契约**（稳定性约束）。Registry 通过 `pkgutil` 自动扫描，无需手动注册。

### 分析算法

新增分析插件同协议模式，继承 `AnalysisPlugin`（`app/plugins/analyzers/base.py`）。
