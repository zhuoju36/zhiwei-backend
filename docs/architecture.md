...
短期（v0.2 / v0.3 / v0.4 / v0.5 / v0.6，**已交付**）：
...
中期（v0.7）：每项目通知通道配置；钉钉/企微/Slack 专属 payload 包装；通知速率限制与去重；modbus_rtu / opcua 适配器；模态分析 / 趋势预测等其它分析插件与 MinIO 存储；3D 模型上传与转换；完整边缘网关进程；审计日志；首次登录强制改密码。
