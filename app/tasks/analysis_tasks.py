...
执行流程：
1. mark_running
2. 从 readings 拉取时序数据 → np.ndarray
3. AnalyzerRegistry.get(plugin).analyze(channel_id, time_range, data, params)
...
async def _run(job_id: int) -> dict[str, Any]:
    ...
    samples = await _fetch_samples(channel_id, start_iso, end_iso)
    if len(samples) < 2:
        async with AsyncSessionLocal() as db:
    ...
    # 上传 NPZ（含完整频谱/幅值）
    key = f"analysis/{channel_id}/{job_id}.npz"
    npz_buf = io.BytesIO()
